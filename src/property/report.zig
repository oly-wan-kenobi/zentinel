...
// `zentinel.pipeline.property_report.v1`. The report shape is frozen; this
// module is the in-tree executable check that distinguishes passing property
// evidence from missing or malformed evidence.
//
// Consumer scope: this is a verification helper with NO `zentinel` runtime
// consumer -- no CLI verb calls `validate()`, and it never participates in a
// mutation run. The property_report contract is pinned in-tree by
...
/// determinism, stability, round-trip, isolation, monotonicity, and
