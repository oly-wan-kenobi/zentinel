...
The `doctest_survivor.source_case` object is a compact closed reference to the original ordinary doctest: `doctest_case_id`, `file`, and `source_ref`. The `doctest_survivor.mutation_case` object is a compact closed reference to the selected survived mutation entry: `case_id`, `mutant_id`, `operator`, `mutated_diff`, `backend_stability`, and `runner_evidence`. `runner_evidence` is copied from `case.mutation.runner_evidence` in the selected mutation-aware doctest report. It is closed and contains `status`, non-null `command`, `exit_code`, `timed_out`, `failure_kind`, `stdout_excerpt`, `stderr_excerpt`, `failure_summary`, and `skip_reason`. Mutation-aware doctest runner evidence must include `command` and `failure_kind` so AI prompts never infer runner command or outcome class from prose; mutation-aware doctest runner evidence is the source of truth for survivor assistance. Examples use illustrative line numbers; executable fixtures must derive source refs from current extraction metadata rather than copying example line numbers.
...
```json
{ "kind": "doctest_survivor", "survivor_ref": "ds_01hr7p6h0v2fj3drdzt9k2a0xe", "source_case": { "doctest_case_id": "dt_01hr7p6h0v2fj3drdzt9k2a0xe", "file": "docs/MUTATOR_SPEC.md", "source_ref": "docs/MUTATOR_SPEC.md:120:range-boundary" }, "mutation_case": { "case_id": "dm_01hr7p6h0v2fj3drdzt9k2a0xe", "mutant_id": "m_01hr7p6h0v2fj3drdzt9k2a0xe", "operator": "comparison_boundary", "mutated_diff": ["- try expect(idx < len);", "+ try expect(idx <= len);"], "backend_stability": "stable", "runner_evidence": { "status": "survived", "command": { "original": "zig test src/doctest.zig", "argv": ["zig", "test", "src/doctest.zig"], "cwd": "<project>", "environment_policy": "minimal", "shell": false }, "exit_code": 0, "timed_out": false, "failure_kind": "none", "stdout_excerpt": "", "stderr_excerpt": "", "failure_summary": "", "skip_reason": null } } }
```
...
- redact command output, command evidence (`original`, `argv`, `cwd`), runner skip reasons, and absolute paths before prompt construction
