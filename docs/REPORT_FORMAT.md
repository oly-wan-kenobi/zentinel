...
Generated same-file commands that were not part of `baseline.commands` must appear in `test_selection.preflight_commands` before they can classify a mutant. Each preflight entry uses the shared structured command object with `phase = "selection_preflight"` and records deterministic status, exit code, timeout flag, failure kind, duration, bounded evidence, and skip reason. A generated selected command may classify a mutant only when its corresponding preflight entry has `status = "passed"`, `failure_kind = "none"`, and `skip_reason = null`; generated command construction/parsing failures are recorded as skipped preflight evidence with a deterministic non-null `skip_reason`.
