...
Failure recovery (`docs/FAILURE_RECOVERY.md`) runs inside these stages: `failed_implementation`, `failed_mutation_gate`, `flaky_verification`, `rollback_required`, `escalated`, `return_to_role`, and `follow_up_created` are recovery artifact stages within the `active` task-control state. Only `blocked` is a task-control state, and a failure stage never transitions directly to `complete`. Task `065` adds `validate_failure_recovery` to `scripts/validate_task_system.py`, which rejects invalid recovery transitions — a failure stage reaching `complete`, retry exhaustion that does not escalate, or a `rollback_required` state without agent-owned changed-file evidence — using `test/fixtures/pipeline/failure_recovery_validator`, so a failed gate cannot be marked complete without auditable recovery evidence.
