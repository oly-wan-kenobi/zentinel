...
The ZIR backend does **real ZIR lowering for every binary-operator mutation**: comparison (`equality_swap`, `comparison_boundary`), short-circuit logical (`logical_and_or`), and arithmetic (`arithmetic_add_sub`, `arithmetic_mul_div`). `src/zir_backend.zig` `fromTree`/`listFromTrees` lowers each source file to ZIR with `std.zig.AstGen.generate` and recognizes mutation sites from the instructions ZIR emits — `cmp_*`, `bool_br_and`/`bool_br_or`, and `add`/`sub`/`mul`/`div` — mapping each back to its exact AST node via the `pl_node.src_node` declaration-relative offset (`expectedAstTag` → `matchInstructions`). Candidate metadata is held in exact differential parity with the AST recognizers (`comparison.zig`, `logical.zig`, `arithmetic.zig`) — pinned by the `zir_backend` parity tests, including `arithmetic_*`'s `expected_compile = .may_fail` — so each ZIR candidate that *is* recognized is byte-identical to its AST counterpart in a runtime context, re-tagged `backend = zir`, `backend_stability = experimental`. Completeness is achieved by maximum bipartite matching (`matchInstructions`, Kuhn's augmenting paths): each recognized instruction is matched to a distinct candidate AST node (an edge where `node - off` is a declaration base), so same- and cross-offset collisions are resolved without stranding a site. Over `src/` the ZIR set equals the AST set exactly — the full-tree oracle sweep asserts `ast_only == 0`. (Earlier resolvers — innermost-base, then claim-aware — dropped ~22% then ~10% of sites; max-matching closed the gap.) Surplus ZIR lowerings — more instructions than the operator has source nodes — match nothing and produce no duplicate candidate. The **one** in-context refinement ZIR adds is comptime-aware `expected_compile`: a binary-operator site inside a `comptime { ... }` block (located via the ZIR `block_comptime` instructions; see `comptimeBlockSpans`) has its `.compiles` bucket downgraded to `.may_fail`, because comptime evaluation is strict (a swap can surface a *compile* error a runtime context would defer). AstGen-injected operators (for-bounds, array indexing, switch ranges) resolve to no source node and become out-of-report diagnostics, never mutants.
...
The backend targets pinned Zig `0.16.0`. Because the `pl_node.src_node` decoding is coupled to that exact toolchain, `listFromTrees` actively declines (`error.UnsupportedZigVersion`, surfaced as a clear `--backend zir` diagnostic) on any other version — including a same-version nightly or a missing Zig — via `toolchainSupported`, rather than risk silent mis-resolution on a moving release. As a standing cross-check, `differentialOracle` compares the ZIR-recognized binary-operator set against the AST recognizers' set; a test sweeps it over `src/` on every `zig build test`, asserting the invariant that ZIR never recognizes a site the AST backend misses — and, with maximum matching, full parity (ZIR == AST, zero residual).
