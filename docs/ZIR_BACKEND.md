...
The backend targets pinned Zig `0.16.0`. Because the `pl_node.src_node` decoding is coupled to that exact toolchain, `listFromTrees` actively declines (`error.UnsupportedZigVersion`, surfaced as a clear `--backend zir` diagnostic) on any other version — including a same-version nightly or a missing Zig — via `toolchainSupported` (3a), rather than risk silent mis-resolution on a moving release. As a standing cross-check, `differentialOracle` compares the ZIR-recognized binary-operator set against the AST recognizers' set; the ZIR-6 test sweeps it over `src/` on every `zig build test`, asserting the invariant that ZIR never recognizes a site the AST backend misses — and, since ZIR-7's maximum matching, full parity (ZIR == AST, zero residual).
