...
- Add failing CLI tests for `zentinel doctest suggest-missing [--file <doc-path>]` with deterministic public-docs metadata, no report present, optional `--report` context, invalid docs paths using `ZNTL_DOCTEST_DOC_NOT_FOUND`, and proof that suggestions do not edit docs.
